...
eBayTradingAPI.prototype.makeRequest = function(name, fields, format, authToken, callback) {
...
function normalizeArrays(json_data) {
    if (json_data instanceof Array) {
        json_data.forEach(function(a) {
            normalizeArrays(a);
        })
    }
    else if (typeof json_data == "object") {
        for (var key in json_data) {
            var matches = key.match(/Array$/)
            if (matches) {
                var subKey = key.substring(0, matches.index)
                json_data[key] = json_data[key][subKey];
               if(!(json_data[key] instanceof Array)) {
                json_data[key] = [ json_data[key] ]
               }
                normalizeArrays(json_data[key]);
            }
        }
    }
}
