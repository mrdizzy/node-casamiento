var _ = require('underscore'),
    db = require('./../config/db').test_ebay, 
    colours = require('./../lib/colour_lookup');
    
module.exports = function(app){
  app.get("/", function(req, res) {
    db.get(["birds_of_paradise_spirals-name_place","foliage-name_place", "whimsical-name_place", "rose-name_place","beautiful-name_place","damask-name_place","wild_vintage-name_place"], function(err, docs) {
      docs = docs.toArray();  
      docs.forEach(function(card) {
      console.log(card)
        card.colour_0 = colours[card.colours[0]]
        var colour_1 = card.colours[1]
        if(colour_1) {
          card.colour_1 = colours[colour_1]
        }
        
      })
      docs.place_cards = docs;
      res.render('welcome/index', {
        layout: 'welcome_layout',
        locals: docs
      });
    });
  })
}